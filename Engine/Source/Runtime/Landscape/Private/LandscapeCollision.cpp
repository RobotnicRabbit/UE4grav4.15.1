...
ULandscapeHeightfieldCollisionComponent::FPhysXHeightfieldRef::~FPhysXHeightfieldRef()
{
	...
	if (RBHeightFieldShapeSync)
	{
		GPhysXPendingKillHeightfieldGeometry.Add(RBHeightFieldShapeSync);
		RBHeightFieldShapeSync = NULL;
	}
	if (RBHeightFieldShapeSimpleSync)
	{
		GPhysXPendingKillHeightfieldGeometry.Add(RBHeightFieldShapeSimpleSync);
		RBHeightFieldShapeSimpleSync = NULL;
	}
	if (RBHeightFieldShapeAsync)
	{
		GPhysXPendingKillHeightfieldGeometry.Add(RBHeightFieldShapeAsync);
		RBHeightFieldShapeAsync = NULL;
	}
	if (RBHeightFieldShapeSimpleAsync)
	{
		GPhysXPendingKillHeightfieldGeometry.Add(RBHeightFieldShapeSimpleAsync);
		RBHeightFieldShapeSimpleAsync = NULL;
	}
#if WITH_EDITOR
	if (RBHeightfieldEd)
	{
	...
	if (RBHeightFieldEdShapeSync)
	{
		GPhysXPendingKillHeightfieldGeometry.Add(RBHeightFieldEdShapeSync);
		RBHeightFieldEdShapeSync = NULL;
	}
#endif// WITH_EDITOR
#endif// WITH_PHYSX
	...
	if (!BodyInstance.IsValidBodyInstance())
	{
		...
		if (IsValidRef(HeightfieldRef))
		{
			...
			if (LandscapeComponentGeom.isValid())
			{
				...
				HeightfieldRef->RBHeightFieldShapeSync = GPhysXSDK->createShape(LandscapeComponentGeom, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
				PxShape* HeightFieldShapeSync = HeightfieldRef->RBHeightFieldShapeSync;
				...
				if (bCreateSimpleCollision)
				{
					...
					HeightfieldRef->RBHeightFieldShapeSimpleSync = GPhysXSDK->createShape(LandscapeComponentGeomSimple, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
					PxShape* HeightFieldShapeSimpleSync = HeightfieldRef->RBHeightFieldShapeSimpleSync;
				...
				if (!GetWorld()->IsGameWorld())
				{
					...
					if (LandscapeComponentGeomEd.isValid())
					{
						...
						HeightfieldRef->RBHeightFieldEdShapeSync = GPhysXSDK->createShape(LandscapeComponentGeomEd, &PDefaultMat, 1, true);
						PxShape* HeightFieldEdShapeSync = HeightfieldRef->RBHeightFieldEdShapeSync;
				...
				if (bHasAsyncScene)
				{
					...
					HeightfieldRef->RBHeightFieldShapeAsync = GPhysXSDK->createShape(LandscapeComponentGeom, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
					PxShape* HeightFieldShapeAsync = HeightfieldRef->RBHeightFieldShapeAsync;
					...
					if (bCreateSimpleCollision)
					{
						PxHeightFieldGeometry LandscapeComponentGeomSimple(HeightfieldRef->RBHeightfieldSimple, PxMeshGeometryFlags(), LandscapeScale.Z * LANDSCAPE_ZSCALE, LandscapeScale.Y * SimpleCollisionScale, LandscapeScale.X * SimpleCollisionScale);
						check(LandscapeComponentGeomSimple.isValid());
						HeightfieldRef->RBHeightFieldShapeSimpleAsync = GPhysXSDK->createShape(LandscapeComponentGeomSimple, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
						PxShape* HeightFieldShapeSimpleAsync = HeightfieldRef->RBHeightFieldShapeSimpleAsync;

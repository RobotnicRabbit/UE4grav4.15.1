...
#include "Particles/Acceleration/ParticleModuleAccelerationWorldGravity.h"
...
/*-----------------------------------------------------------------------------
+	ParticleModuleAccelerationWorldGravity implementation.
+-----------------------------------------------------------------------------*/

UParticleModuleAccelerationWorldGravity::UParticleModuleAccelerationWorldGravity(const FObjectInitializer& ObjectInitializer)
 : Super(ObjectInitializer)
{
	bSpawnModule = true;
	bUpdateModule = true;
...
void UParticleModuleAccelerationWorldGravity::Spawn(FParticleEmitterInstance* Owner, int32 Offset, float SpawnTime, FBaseParticle* ParticleBase)
{
	SPAWN_INIT;
	UParticleLODLevel* LODLevel = Owner->SpriteTemplate->GetCurrentLODLevel(Owner);
	check(LODLevel);
	...
	if (World != NULL)
	{
		Acceleration = World->GetGravity() * GravityScale;
	}
	if (bAlwaysInWorldSpace && LODLevel->RequiredModule->bUseLocalSpace)
	{
		FVector LocalAcceleration = Owner->Component->ComponentToWorld.InverseTransformVector(Acceleration);
...
void UParticleModuleAccelerationWorldGravity::Update(FParticleEmitterInstance* Owner, int32 Offset, float DeltaTime)
{
	if ((Owner == NULL) || (Owner->ActiveParticles <= 0) ||
		(Owner->ParticleData == NULL) || (Owner->ParticleIndices == NULL))
	{
...
bool UParticleModuleAccelerationWorldGravity::IsValidForLODLevel(UParticleLODLevel* LODLevel, FString& OutErrorString)
 {
	if (LODLevel->TypeDataModule && LODLevel->TypeDataModule->IsA(UParticleModuleTypeDataGpu::StaticClass()))
	{
		/*

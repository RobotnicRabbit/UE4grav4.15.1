...
FVector AWorldSettings::GetGravity() const
{
	if (!bWorldGravitySet)
	{
		...
		MutableThis->WorldGravity = bGlobalGravitySet ? GlobalGravity : UPhysicsSettings::Get()->DefaultGravity;   //allows us to override DefaultGravity
	}
...
void AWorldSettings::SetGravity(FVector NewGravity)
{
	GlobalGravity = NewGravity;
	bGlobalGravitySet = true;
	
	for (TObjectIterator<UStaticMeshComponent> MeshItr; MeshItr; ++MeshItr)
	{
		MeshItr->WakeAllRigidBodies();
	}
}
...
void AWorldSettings::Serialize( FArchive& Ar )
{
	...
	{
		//before we had override flag we would use GlobalGravity != ZeroVector

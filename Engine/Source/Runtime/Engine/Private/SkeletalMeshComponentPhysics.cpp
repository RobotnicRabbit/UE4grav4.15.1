...
void USkeletalMeshComponent::SetUseCustomGravity(bool bCustomGravityEnabled)
{
	if (!bEnablePhysicsOnDedicatedServer && IsRunningDedicatedServer())
	{
		return;
	...
	BodyInstance.bUseCustomGravity = bCustomGravityEnabled;
	...
	if (UPhysicsAsset * PhysAsset = GetPhysicsAsset())
	{
		for (int32 BodyIdx = 0; BodyIdx < Bodies.Num(); ++BodyIdx)
		{
			if (FBodyInstance* BodyInstance = Bodies[BodyIdx])
			{
				if (UBodySetup * PhysAssetBodySetup = PhysAsset->SkeletalBodySetups[BodyIdx])
				{
					bool bHasCustomGravity = bCustomGravityEnabled;
					
					//If the default body instance has gravity turned off then turning it ON for skeletal mesh component does not turn the instance on
					if (bHasCustomGravity && !PhysAssetBodySetup->DefaultInstance.bUseCustomGravity)
					{
						bHasCustomGravity = false;
					}
					
					BodyInstance->SetUseCustomGravity(bHasCustomGravity);
				}
			}
		}
	}
...
bool USkeletalMeshComponent::IsCustomGravityEnabled() const
{
	return BodyInstance.bEnableGravity && BodyInstance.bUseCustomGravity;
}

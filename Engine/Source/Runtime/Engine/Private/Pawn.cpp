...
void APawn::MoveForward2(float ScaleValue /*= 1.0f*/, FVector ActorUpVector, FVector ForwardCam, float AirControl, bool bForce /*= false*/)
{
	//UPawnMovementComponent* MovementComponent = GetMovementComponent();
	if (UPawnMovementComponent*MovementComponent = NULL) { return; }

	const FVector UpDirection = ActorUpVector;
	const FVector CameraForward = ForwardCam;
	...
	{
		CurrentForwardDirection = FVector::VectorPlaneProject(ForwardCam, ActorUpVector);
	}
	UPawnMovementComponent* MovementComponent = GetMovementComponent();
	const float ControlValue = MovementComponent->IsMovingOnGround() ? ScaleValue : ScaleValue * AirControl;
	AddMovementInput(CameraForward.GetSafeNormal(), ControlValue, bForce);


}
/*
void APawn::Align_To_Floor(AActor const* HitActor, AActor const* HitInstigator, FHitResult& OutHitInfo, FVector& OutImpulseDir) const
{
	ensure(HitActor);
	if (HitActor)
	{

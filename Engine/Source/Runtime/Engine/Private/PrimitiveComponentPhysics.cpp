...
void UPrimitiveComponent::SetUseCustomGravity(bool useCustomGravity)
{
	FBodyInstance* BI = GetBodyInstance();
	if (BI)
	{
		BI->SetUseCustomGravity(useCustomGravity);
	}
}
...
bool UPrimitiveComponent::IsCustomGravityEnabled() const
{
	FBodyInstance* BI = GetBodyInstance();
	if (BI)
	{
		return BI->bEnableGravity && BI->bUseCustomGravity;
	}
	
	return false;
}

FVector UPrimitiveComponent::GetGravity() const
{
	FBodyInstance* BI = GetBodyInstance();
	if (BI)
	{
		if (IsCustomGravityEnabled())
		{
...
void UPrimitiveComponent::SetCustomGravity(FVector NewGravity)
{
	...
	if (BI)
	{
		BI->SetCustomGravity(NewGravity);
	}
}

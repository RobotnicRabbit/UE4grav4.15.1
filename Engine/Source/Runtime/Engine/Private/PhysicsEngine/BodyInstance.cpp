...
{
	CustomGravityTick.bCanEverTick = true;
	CustomGravityTick.bStartWithTickEnabled = false;
	CustomGravityTick.TickGroup = TG_PrePhysics;
	
	OnCustomGravityPhysics.BindRaw(this, &FBodyInstance::CustomGravitySubstep);
...
{
	InstanceBodyIndex = Other.InstanceBodyIndex;
	InstanceBoneIndex = Other.InstanceBoneIndex;
	Scale3D = Other.Scale3D;
	SceneIndexSync = Other.SceneIndexSync;
	SceneIndexAsync = Other.SceneIndexAsync;
	CollisionProfileName = Other.CollisionProfileName;
	CollisionResponses = Other.CollisionResponses;
	MaskFilter = Other.MaskFilter;
	bUseCCD = Other.bUseCCD;
	bNotifyRigidBodyCollision = Other.bNotifyRigidBodyCollision;
	bSimulatePhysics = Other.bSimulatePhysics;
	bOverrideMass = Other.bOverrideMass;
	bEnableGravity = Other.bEnableGravity;
	bUseCustomGravity = Other.bUseCustomGravity;
	bAutoWeld = Other.bAutoWeld;
	bWelded = Other.bWelded;
	bStartAwake = Other.bStartAwake;
	bGenerateWakeEvents = Other.bGenerateWakeEvents;
	bUpdateMassWhenScaleChanges = Other.bUpdateMassWhenScaleChanges;
	bLockTranslation = Other.bLockTranslation;
	bLockRotation = Other.bLockRotation;
	bLockXTranslation = Other.bLockXTranslation;
	bLockYTranslation = Other.bLockYTranslation;
	bLockZTranslation = Other.bLockZTranslation;
	bLockXRotation = Other.bLockXRotation;
	bLockYRotation = Other.bLockYRotation;
	bLockZRotation = Other.bLockZRotation;
	bOverrideMaxAngularVelocity = Other.bOverrideMaxAngularVelocity;
	bWokenExternally = Other.bWokenExternally;
	bUseAsyncScene = Other.bUseAsyncScene;
	bOverrideMaxDepenetrationVelocity = Other.bOverrideMaxDepenetrationVelocity;
	bOverrideWalkableSlopeOnInstance = Other.bOverrideWalkableSlopeOnInstance;
	bHasSharedShapes = Other.bHasSharedShapes;
	MaxDepenetrationVelocity = Other.MaxDepenetrationVelocity;
	ExternalCollisionProfileBodySetup = Other.ExternalCollisionProfileBodySetup;
	MassInKgOverride = Other.MassInKgOverride;
	CustomGravity = Other.CustomGravity;
	LinearDamping = Other.LinearDamping;
	AngularDamping = Other.AngularDamping;
	CustomDOFPlaneNormal = Other.CustomDOFPlaneNormal;
	COMNudge = Other.COMNudge;
	MassScale = Other.MassScale;
	DOFConstraint = Other.DOFConstraint;
	WeldParent = Other.WeldParent;
	WalkableSlopeOverride = Other.WalkableSlopeOverride;
	PhysMaterialOverride = Other.PhysMaterialOverride;
	MaxAngularVelocity = Other.MaxAngularVelocity;
	CustomSleepThresholdMultiplier = Other.CustomSleepThresholdMultiplier;
	PhysicsBlendWeight = Other.PhysicsBlendWeight;
	PositionSolverIterationCount = Other.PositionSolverIterationCount;

#if WITH_PHYSX
	RigidActorSync = Other.RigidActorSync;
	RigidActorAsync = Other.RigidActorAsync;
	BodyAggregate = Other.BodyAggregate;
	CharDebugName = Other.CharDebugName;
	PhysxUserData = Other.PhysxUserData;
	ShapeToBodiesMap = Other.ShapeToBodiesMap;
#endif	//WITH_PHYSX

	RigidActorSyncId = Other.RigidActorSyncId;
	RigidActorAsyncId = Other.RigidActorAsyncId;
	VelocitySolverIterationCount = Other.VelocitySolverIterationCount;
	InitialLinearVelocity = Other.InitialLinearVelocity;
	OwnerComponent = Other.OwnerComponent;
	BodySetup = Other.BodySetup;
	CustomGravityTick = Other.CustomGravityTick;
	OnCustomGravityPhysics = Other.OnCustomGravityPhysics;
	OnCalculateCustomProjection = Other.OnCalculateCustomProjection;
	OnRecalculatedMassProperties = Other.OnRecalculatedMassProperties;

#if WITH_BOX2D
	BodyInstancePtr = Other.BodyInstancePtr;
#endif	//WITH_BOX2D

	CurrentSceneState = Other.CurrentSceneState;
	SleepFamily = Other.SleepFamily;
	DOFMode = Other.DOFMode;
...
FBodyInstanceCustomGravityTickFunction::FBodyInstanceCustomGravityTickFunction()
{
	Target = NULL;
}

FBodyInstanceCustomGravityTickFunction & FBodyInstanceCustomGravityTickFunction::operator=(const FBodyInstanceCustomGravityTickFunction& Other)
{
	Target = Other.Target;
...
void FBodyInstanceCustomGravityTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	if (Target)
	{
		Target->TickCustomGravity(DeltaTime, TickType, *this);
	}
}

FString FBodyInstanceCustomGravityTickFunction::DiagnosticMessage()
{
	return Target->GetBodyDebugName() + TEXT("[CustomGravityTick]");
}

void FBodyInstance::TickCustomGravity(float DeltaTime, enum ELevelTick TickType, FBodyInstanceCustomGravityTickFunction& ThisTickFunction)
{
	if (bEnableGravity && bUseCustomGravity)
	{
		AddCustomPhysics(OnCustomGravityPhysics);
	}
}

void FBodyInstance::CustomGravitySubstep(float DeltaTime, FBodyInstance* BodyInstance)
{
	if (BodyInstance->bEnableGravity && BodyInstance->bUseCustomGravity)
	{
		BodyInstance->AddForce(BodyInstance->CustomGravity, NAME_None, true);
	}
}

void FBodyInstance::RegisterCustomGravityTickFunction(bool bRegister)
{
	const UPrimitiveComponent* OwnerComponentInst = OwnerComponent.Get();
	if (OwnerComponentInst)
	{
		if (bRegister && OwnerComponentInst->GetComponentLevel())
		{
			if (CustomGravityTick.bCanEverTick)
			{
				CustomGravityTick.Target = this;
				CustomGravityTick.SetTickFunctionEnable(CustomGravityTick.bStartWithTickEnabled || CustomGravityTick.IsTickFunctionEnabled());
				CustomGravityTick.RegisterTickFunction(OwnerComponentInst->GetComponentLevel());
			}
		}
		else
		{
			if (CustomGravityTick.IsTickFunctionRegistered())
			{
				CustomGravityTick.UnRegisterTickFunction();
			}
		}
	}
}

void FBodyInstance::SetCustomGravityTickEnabled(bool bEnabled)
{
	if (CustomGravityTick.bCanEverTick && bSimulatePhysics)
	{
		CustomGravityTick.SetTickFunctionEnable(bEnabled);
	}
}

bool FBodyInstance::IsCustomGravityTickEnabled() const
{
	return CustomGravityTick.IsTickFunctionEnabled();
}
	...
	bool CreateShapesAndActors_PhysX(TArray<PxActor*>& PSyncActors, TArray<PxActor*>& PAsyncActors, TArray<PxActor*>& PDynamicActors, const bool bCanDefer, bool& bDynamicsUseAsyncScene) 
	{
		...
		for (int32 BodyIdx = NumBodies - 1; BodyIdx >= 0; BodyIdx--)   // iterate in reverse since list might shrink
		{
			...
			if (!IsStatic() && PNewDynamic)
			{
				...
				if (!Instance->bEnableGravity || Instance->bUseCustomGravity)
				{
					ModifyActorFlag_Isolated<PxActorFlag::eDISABLE_GRAVITY>(PNewDynamic, true);
				}
				Instance->RegisterCustomGravityTickFunction(true);
				Instance->SetCustomGravityTickEnabled(Instance->bEnableGravity && Instance->bUseCustomGravity);
...
void FBodyInstance::TermBody()
{
	...
	if (BodyInstancePtr != NULL)
	{
		...
		{
			RegisterCustomGravityTickFunction(false);
...
void FBodyInstance::CopyBodyInstancePropertiesFrom(const FBodyInstance* FromInst)
{
	...
	check(FromInst);
	check(FromInst->CustomGravityTick.Target == NULL);
...
void FBodyInstance::SetContactReportForceThreshold(float Threshold)
{
}
...
void FBodyInstance::SetEnableGravity(bool bInGravityEnabled)
{
	if (bEnableGravity != bInGravityEnabled)
	{
		...
		const bool actuallyEnableGravity = bEnableGravity && !bUseCustomGravity;

#if WITH_PHYSX
		ExecuteOnPxRigidBodyReadWrite(this, [&](PxRigidBody* PRigidBody)
		{
			ModifyActorFlag_Isolated<PxActorFlag::eDISABLE_GRAVITY>(PRigidBody, !actuallyEnableGravity);
		});
#endif // WITH_PHYSX
		...
		if (BodyInstancePtr)
		{
			BodyInstancePtr->SetGravityScale(actuallyEnableGravity ? 1.0f : 0.0f);
		}
#endif
		...
		if (bEnableGravity)
		{
			...
			SetCustomGravityTickEnabled(bUseCustomGravity);
		}
		else
		{
			SetCustomGravityTickEnabled(false);
		}
	}
}

void FBodyInstance::SetUseCustomGravity(bool bCustomGravityEnabled)
{
	if (bUseCustomGravity != bCustomGravityEnabled)
	{
		bUseCustomGravity = bCustomGravityEnabled;
		
		const bool enableGravity = bEnableGravity && !bUseCustomGravity;
		
		#if WITH_PHYSX
		ExecuteOnPxRigidBodyReadWrite(this, [&](PxRigidBody* PRigidBody)
		{
			PRigidBody->setActorFlag(PxActorFlag::eDISABLE_GRAVITY, !enableGravity);
		});
		#endif // WITH_PHYSX
		...
		if (BodyInstancePtr)
		{
			BodyInstancePtr->SetGravityScale(enableGravity ? 1.0f : 0.0f);
		}
		#endif
		...
		{
			WakeInstance();
			SetCustomGravityTickEnabled(true);
		}
		else
		{
			SetCustomGravityTickEnabled(false);
...
void FBodyInstance::SetCustomGravity(FVector NewGravity)
{
	CustomGravity = NewGravity;
}

FVector FBodyInstance::GetGravity()
{
	if (bEnableGravity) {
		...
		const UPrimitiveComponent* OwnerComponentInst = OwnerComponent.Get();
		return OwnerComponentInst ? OwnerComponentInst->GetWorld()->GetGravity() : FVector::ZeroVector;

...
void DeferredPhysResourceCleanup()
{
	...
	// Release all heightfields and other data in PxGeometry and PxShape and reset array
	for (int32 HfIdx = 0; HfIdx<GPhysXPendingKillHeightfieldGeometry.Num(); HfIdx++)
	{
		PxShape* PHeightfieldGeom = GPhysXPendingKillHeightfieldGeometry[HfIdx];

		// Check this as it shouldn't be null, but then gate on it so we can
		// avoid a crash if we end up in this state in shipping
		check(PHeightfieldGeom);
		if (PHeightfieldGeom)
		{
			PHeightfieldGeom->release();

			if (GPhysXPendingKillHeightfieldGeometry.IsValidIndex(HfIdx))
			{
				GPhysXPendingKillHeightfieldGeometry[HfIdx] = NULL;
			}
			else
			{
				UE_LOG(LogPhysics, Warning, TEXT("DeferredPhysResourceCleanup found invalid index into GPhysXPendingKillHeightfieldGeometry (%d), another thread may have modified the array."), HfIdx);
			}
		}
		else
		{
			UE_LOG(LogPhysics, Warning, TEXT("DeferredPhysResourceCleanup found null PHeightfieldGeom in pending kill array (at %d), another thread may have modified the array."), HfIdx);
		}
	}
	GPhysXPendingKillHeightfieldGeometry.Reset();

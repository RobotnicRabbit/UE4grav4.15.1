...
/**
 * Tick function that calls FBodyInstance::TickCustomGravity
 **/
USTRUCT()
struct FBodyInstanceCustomGravityTickFunction : public FTickFunction
{
	GENERATED_USTRUCT_BODY()
	
	/**  FBodyInstance  that is the target of this tick **/
	struct FBodyInstance* Target;

	FBodyInstanceCustomGravityTickFunction();

	FBodyInstanceCustomGravityTickFunction& operator=(const FBodyInstanceCustomGravityTickFunction& Other);
...
public:
	...
	/** Enables/disables whether this body has a custom gravity */
	void SetUseCustomGravity(bool bCustomGravityEnabled);
	/** Sets CustomGravity */
	void SetCustomGravity(FVector NewGravity);
	/** Gets the gravity acting on this object */
	FVector GetGravity();

	/** custom gravity tick function for this body instance, calls TickCustomGravityTick */
	UPROPERTY()
	struct FBodyInstanceCustomGravityTickFunction CustomGravityTick;

	/** Tick function called to set custom gravity*/
	virtual void TickCustomGravity(float DeltaTime, enum ELevelTick TickType, FBodyInstanceCustomGravityTickFunction& ThisTickFunction);

	FCalculateCustomPhysics OnCustomGravityPhysics;

	void CustomGravitySubstep(float DeltaTime, FBodyInstance* BodyInstance);

	/**
	 * When called, will call the virtual call chain to register the custom gravity tick function
	 * Do not override this function or make it virtual
	 * @param bRegister - true to register, false, to unregister
	 */
	void RegisterCustomGravityTickFunction(bool bRegister);

	/**
	 * Set this body instance's custom gravity tick functions to be enabled or disabled. Only has an effect if the function is registered
	 * This only modifies the tick function on body instance itself
	 * @param	bEnabled - Rather it should be enabled or not
	 */
	void SetCustomGravityTickEnabled(bool bEnabled);

	/**  Returns whether this body instance has tick enabled or not	 */
	bool IsCustomGravityTickEnabled() const;
	
	/** accessor for the value of bCanEverTick */
	FORCEINLINE bool CanEverTick() const { return CustomGravityTick.bCanEverTick; }

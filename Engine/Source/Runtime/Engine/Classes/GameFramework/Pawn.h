...
public:
	...
	*   Subclasses may check this as well if they override TakeDamage and don't want to potentially trigger TakeDamage actions by checking if it returns zero in the super class. */
	...
	/**
	 * Returns the normalized direction of gravity
	 * aka gravity normalized
	 *
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Pawn|Input", meta = (Keywords = "AddInput"))
		void MoveForward2(float ScaleValue, FVector ActorUpVector, FVector ForwardCam, float AirControl, bool bForce);


	//UFUNCTION(BlueprintCallable, Category = "Pawn|Input", meta = (Keywords = "AddInput"))
	//void Align_To_Floor(AActor const * HitActor, AActor const * HitInstigator, FHitResult & OutHitInfo, FVector & OutImpulseDir) const;

	//virtual void AddMovementInput2(float ScaleValue /*= 1.0f*/, FVector ActorUpVector, FVector ForwardCam, float AirControl, bool bForce /*= false*/);
...
public:
	...
	//new movement
	/** Internal function meant for use only within Pawn or by a PawnMovementComponent. Adds movement input if not ignored, or if forced. */
	//void Internal_AddMovementInput2(float scaleval, FVector Actorup, FVector Camforward, float aircontrol, bool bForce = false);
